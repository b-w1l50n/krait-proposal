import { proposalPages } from './proposal.js';
...
heading('02 / MATLAB · RATE & SCHEDULING STUDY','Coordination is a timing problem','Compare scheduling with baseline route predictions against a stress test with prediction errors and added disturbances.')+
...
<div class="table-scroll"><table><caption>Same 190 dB scenario · fresh model-accepted records and update spacing</caption><thead><tr><th>Scheduling approach / conditions</th><th>Accepted</th><th>P95 spacing</th><th>Worst spacing</th></tr></thead><tbody><tr><td>Shared low-frequency band — baseline</td><td>57.94%</td><td>5.01 s</td><td>10.19 s</td></tr><tr><td>Fixed frequency-group assignment — baseline</td><td>82.10%</td><td>6.45 s</td><td>31.12 s</td></tr><tr><td>Route-based scheduling — baseline predictions</td><td>99.88%</td><td>3.17 s</td><td>5.30 s</td></tr><tr class="warning-row"><td>Route-based scheduling — with prediction errors</td><td>94.38%</td><td>3.17 s</td><td>185.16 s</td></tr></tbody></table></div>
...
heading('03 / MATLAB · SAMPLED RECEIVER TRIALS','Power assumptions change the result','Earlier waveform trials expose a constraint that a favorable scheduling model cannot remove.')+
...
heading('04 / SPICE & IMPLEMENTATION SCREENING','Evidence for the next prototype','Simulation narrows the design space. Physical validation remains the next step.')+
...
<div class="two-notes"><article><span class="eyebrow">ANALOG LIMITS</span><h2>Headroom comes first.</h2><p>The assumed 0.5 A power stage clips demanding upper-band and composite loads. Selected converter settling, noise, thermal behavior and transducer loading still need bench evidence.</p><p>One favorable generic ADC load cleared a half-LSB tracking comparison but retained roughly −57 dBc nonfundamental residual in a severe reset test. That does not establish 16-bit effective resolution.</p></article><article><span class="eyebrow">IMPLEMENTATION LIMITS</span><h2>Parts of the workload, not the whole modem.</h2><p>A limited arithmetic probe was synthesized. No complete modem synthesis, placement, routing, timing closure or board programming is claimed.</p><p>Storage capacity cases fit the candidate memory allocation, but sustained interface throughput and complete implementation deadlines remain unmeasured.</p></article></div>
`+
...
heading('05 / DEVELOPMENT PROPOSAL','A staged path to real hardware','The study supports prototype development. It does not establish a qualified product or a finished manufacturing cost.')+
`<div class="metric-row">${metric('Below $500','BOM target · each modem','Preliminary target for both follower and coordinator. Component selection is unfinished.')}${metric('2–3 km','Desired operating range','A requirement for validation, not experimentally proven range.')}${metric('20–30','Commercial fleet target','Operating vehicles, not guaranteed simultaneous independent links. Forty remains an aspiration.')}</div>
<div class="result-note"><h2>Designed with scale in mind.</h2><p>Both modem variants target a BOM below <b>$500 each</b>. Final component selection and the complete BOM are still in development.</p><p>We are currently inquiring with custom-transducer manufacturers for quotes. Final transducer pricing may change the preliminary BOM target. We expect OEM volume pricing to be a factor in final per-unit costs. These are preliminary targets, not finalized BOM totals or supplier quotations. The engineering fee, prototype procurement and separately scoped host/backhaul or passive-array work are distinct.</p></div>
<ol class="milestones"><li><span>A</span><div><h3>Agree the acceptance criteria</h3><p>Freeze operating conditions, update deadlines, error targets and supplier evidence.</p></div></li><li><span>B</span><div><h3>Prove the combined digital prototype</h3><p>Exercise waveform behavior, scheduling, handovers and uncertainty together.</p></div></li><li><span>C</span><div><h3>Measure the electronics</h3><p>Validate converter settling, output limits, leakage, memory throughput and FPGA timing.</p></div></li><li><span>D</span><div><h3>Build an integrated Rev A</h3><p>Review schematics, the real PCB layout, manufacturing files and a revised quoted BOM.</p></div></li><li><span>E</span><div><h3>Test in water, then revise</h3><p>Progress from controlled-water tests to field trials against the agreed thresholds.</p></div></li></ol>
<p class="study-context">No experimentally validated KRAIT modem performance is shown. A $15,000 offer for PCB design, basic modem code and hardware-in-the-loop (HIL) bench validation of the custom modems is described in the For Review chapter. Dates and acceptance criteria remain subject to agreement.</p>`+
source('krait/deliverables/KRAIT_modem_feasibility_report.md; krait/bom/proposal_revision/cost_detail.csv; krait/deliverables/KRAIT_contractor_response.md','The below-$500 target for each modem is the user’s latest planning direction and supersedes the older cost presentation. It is not a result calculated from the incomplete study BOM. OEM volume pricing is expected to affect final costs; detailed parts and implementation files remain private.')}
];
